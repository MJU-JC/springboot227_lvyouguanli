<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="分享编号" prop="fenxiangbianhao">
            <el-input v-model="ruleForm.fenxiangbianhao" 
                placeholder="分享编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fenxiangbianhao" label="分享编号" prop="fenxiangbianhao">
              <el-input v-model="ruleForm.fenxiangbianhao" 
                placeholder="分享编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="游记标题" prop="youjibiaoti">
          <el-input v-model="ruleForm.youjibiaoti" 
              placeholder="游记标题" clearable  :readonly="ro.youjibiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="游记标题" prop="youjibiaoti">
              <el-input v-model="ruleForm.youjibiaoti" 
                placeholder="游记标题" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='fenxiangbianhao'){
            this.ruleForm.fenxiangbianhao = obj[o];
	    this.ro.fenxiangbianhao = true;
            continue;
          }
          if(o=='youjibiaoti'){
            this.ruleForm.youjibiaoti = obj[o];
	    this.ro.youjibiaoti = true;
            continue;
          }
          if(o=='fengmian'){
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springboot99u10/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `youjifenxiang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.youjifenxiangCrossAddOrUpdateFlag = false;
						 this.parent.search();
